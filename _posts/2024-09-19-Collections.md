...
유형|설명|
---|---|
Linked|보다 순서를 강조합니다. Set, Map과 같이 기존에 순서가 없었던 컬렉션에는 순서가 생기고, List처럼 이미 순서가 있었던 컬렉션에는 순서를 더욱 강조합니다.<br><br>기존의 List는 순서에 따른 값의 입출력만 가능했고, 순서 중간에 다른 값을 삽입하는 것은 불가능했습니다.\[1,2,3,4,5\] <br><br>따라서 중간에 끼워 넣고 싶으면 새로운 리스트를 만들고, 거기에 기존 리스트에서 끼워 넣고자 하는 순서의 앞 부분을 입력한 뒤, 끼워 넣을 내용을 입력하고, 끼워 넣고자 하는 순서의 뒷 부분을 입력하는 복잡한 절차를 거쳐야 했습니다.<br>하지만 LinkedList는 그러한 과정 없이 바로 끼워 넣기가 가능합니다.\[1,2,9,3,4,5\] |
Hash|검색에 사용되는 값을 hash값으로 보관 및 이용하여 보다 빠른 검색이 가능해집니다. 하지만 아주 가끔 hash값이 중복되어 오류가 발생할 수 있으니, 주의가 필요합니다.|
Tree|순서가 있는 값을 자동으로 정렬해줍니다. 예를 들어, \[5,3,8,1,2\]과 같은 순서로 값을 저장했다면 tree은 자동으로 \[1,2,3,5,8\]로 정렬해줍니다.|
Vector,<br>Table|자바는 기본적으로 멀티스레드를 지원하며, 비동기 방식으로 대량의 데이터를 처리하다 보면 순서를 따르지 않는 경우가 발생하기도 합니다. 예를 들어, `for(i=0; i<100000; i++){list.add(i)}`와 같은 방법으로 값을 저장했다고 가정합니다. 이 때, \[1,2,3,9,4,5,6...\] 과 같은 순서로 저장될 수도 있습니다. 하지만 Vector와 Table은 데이터를 동기적으로 처리하기 때문에 \[1,2,3,4,5,6...\] 으로 저장할 수 있습니다. |
