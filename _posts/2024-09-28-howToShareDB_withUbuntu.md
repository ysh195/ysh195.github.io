...
그리고 이유는 잘 모르겠지만 스프링부트 기준으로 `application.properties` - `DB` 관련 설정에 `spring.jpa.properties.hibernate.dialect=org.hibernate.dialect.MySQLDialect` 이 내용이 반드시 들어가야 합니다.
